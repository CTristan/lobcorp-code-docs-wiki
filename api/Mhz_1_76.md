...
description: 
published: true
date: 2026-01-06T14:23:58.425Z
tags: 
editor: markdown
dateCreated: 2026-01-06T04:23:39.835Z
---
...
## Inheritance
[object](https://learn.microsoft.com/dotnet/api/system.object) → [CreatureBase](/api/CreatureBase) → Mhz_1_76
...
On meltdown, start making noise areas, if not already doing so. Increases the volume over time.
...
Reduces the Qliphoth counter if the agent panics, and at an 80% chance for bad results and 40% for normal results.

Increases the Qliphoth counter by 1 if the result is good, and by an additional 1 point if the result is perfect.

If the Qliphoth counter is at max, stops the noise effects in every area and stops playing sound.

At the end, clears the working agent.
...
Sets the animation script [Mhz_1_76Anim](/api/Mhz_1_76Anim).
...
Increases work probability for agents with low Fortitude.

Multiplies the work probability by 0.3*(5-Fortitude), without reducing it. (This ends up being 1.2x for Fortitude 1, and nothing for anything else...)
